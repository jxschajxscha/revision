class RevisionPlanner {
    ...
    async saveProgress() {
        try {
            await db.collection('revisionData').doc(this.userId).set({
                completedSessions: Array.from(this.completedSessions),
    ...
    async saveCustomNames() {
        try {
            await db.collection('revisionData').doc(this.userId).set({
                customSessionNames: this.customSessionNames,
                lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
            }, { merge: true });
        } catch (error) {
    ...
    // Firebase load methods
    async loadProgress() {
        try {
            const doc = await db.collection('revisionData').doc(this.userId).get();
            if (doc.exists) {
                const data = doc.data();
                if (data.completedSessions) {
                    this.completedSessions = new Set(data.completedSessions);
                ...
                if (data.sessionOrder) {
                    this.loadSessionOrderFromData(data.sessionOrder);
                }
            }
        } catch (error) {
    ...
    async loadCustomNames() {
        try {
            ...
            if (doc.exists) {
                const data = doc.data();
                if (data.customSessionNames) {
                    this.customSessionNames = data.customSessionNames;
        ...
        try {
            ...
            if (doc.exists) {
                ...
                if (data.customReminders) {
                    this.customReminders = data.customReminders;
                }
            }
        } catch (error) {
    ...
    async resetProgress() {
        ...
        try {
            await db.collection('revisionData').doc(this.userId).delete();
        } catch (error) {
